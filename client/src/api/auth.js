...
export async function login(userData) {
    ...
    try {
        const res = await Axios.post(`${process.env.REACT_APP_BACKEND}/login`, userData);
...
export async function signUp(userData) {
    ...
    try {
        const res = await Axios.post(`${process.env.REACT_APP_BACKEND}/signup`, userData);
        console.log("Server response:", res.data.res);
        if (res.data.res === `Error: data incomplete`) {
            return ("Please fill all fields before you signup");
        }
...
export async function authenticateToken() {
    ...
    try {
        const res = await Axios.post(`${process.env.REACT_APP_BACKEND}/authenticate`, {token: token});
        console.log("Server response:", res.data.res);
